...
/* Define se a próxima instrução MEPA usará um rótulo ou não */
...
/* Inicializa variaveis globais do compilador */
    programa:   {   rotulo = 0; nl=1; flag_var=0; pushInt(&declarou,0);
                    ...
                    bloco PONTO {   if(popInt(&declarou))
...
/* DECLARAÇÃO DE VARIÁVEIS */
    parte_declara_vars:     VAR { desloc=0; }           // "desloc" conta o número de variáveis neste escopo
                            declara_vars
                        {   argCodigo("AMEM ",desloc);  // Aloca espaço para variáveis locais
                            declarou->val = 1;          // Constata que houve declaração para poder dar DMEM depois
                            pushInt(&nvars,desloc);}    // Salva número de variáveis locais
                        |
    ;
    ...
    declara_var : { num_idents=0;}                      // "num_idents" conta o número de vars com um mesmo tipo
...
/* ATRIBUIÇÃO */
    ...
    recipiente:   IDENT {   s = buscaSimb(tabela,token2);               // Busca token  na tabela de símbolos
                            if ( s == NULL ) erro(VN_DECL);
                                ...
                                pushInt(&tipos,s->var.tipo);            // Empilha o tipo da variável
                            } else if ( s->cat == CFUNC ){
